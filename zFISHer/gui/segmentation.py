...
class SegmentationGUI(tk.Frame):
    ...
    def highlight_polygon(self, polygon) -> None:
        """
        Colors the fill of the polygon when the mouse hovers over it.

        Args:
        polygon: (tkinter polygon) = tkinter drawn widget
        """

        self.canvas.itemconfig(polygon, fill='yellow')
    def unhighlight_polygon(self, polygon):
        """
        Uncolors the fill of the polygon when the mouse hovers over it.
    ...
    def segment_mips(self):
        """
        Main function to initialize and segment designated mips
        with designated segmentation algorithm 
        """
        self.f1_seg_c = cfg.F1_SEG_C
        self.f2_seg_c = cfg.F2_SEG_C


        self.nuclei_contours = None  

        script = cfg.NUC_SEG_ALGO_PATH
        data = self.f1_seg_c
        print(f"ORIGINAL DATA {data}")
        new_data = self.run_script_with_exec(script,data)
    ...
    def run_script_with_exec(self,script_path, data):
        """
        Executes a Python script and injects data as a global variable.
        
        Args:
        - script_path: Path to the Python script to run.
        - data: Data to inject into the script (can be a dictionary, etc.).
        
        Returns:
        - data: The modified data after executing the script.
        """
        try:
            # Open and read the script content
            with open(script_path, 'r') as file:
                script_content = file.read()

            # Prepare the environment to execute the script
            # Inject 'data' into the script's global environment
            exec(script_content, {"data": data})
